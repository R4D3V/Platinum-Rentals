...
export default function EditLandListingPage({
  params,
}: {
  ...
  const [form, setForm] = useState<Land | null>(null);
  const [saving, setSaving] = useState(false);
  const [featureInput, setFeatureInput] = useState("");

  const { data: landData, isLoading: loading } = useSWR(`/api/user/lands/${id}`, fetcher);
  ...
  if (landData && !form) {
    setForm({
      ...
      titleDocument: landData.titleDocument,
      gradient: landData.gradient,
      images: landData.images ?? [],
      featured: landData.featured ?? false,
    });
  }

  if (!loading && !landData && !form) {
    router.push("/dashboard/lands");
  }

  const areas = useMemo(
    () => {
      const current = form?.area ? [form.area] : [];
      return [...new Set([...current, ...allLands.map((l) => l.area)])].sort();
    },
    [allLands, form?.area],
  );
