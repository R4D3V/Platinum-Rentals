...
export default function AdminLandsPage() {
  const { data: lands = [], isLoading: propsLoading, mutate } = useSWR<Land[]>("/api/admin/lands", fetcher);
  ...
  const userOptions = useMemo(
    () => [...new Set(lands.map((l) => l.landlordId).filter(Boolean))] as string[],
    [lands],
  );
  ...
  const filtered = useMemo(
    () => lands.filter((l) => {
      ...
      if (activeUser && l.landlordId !== activeUser) return false;
      return true;
    }),
    [lands, activeType, activeArea, activeStatus, activeUser],
  );
  ...
  async function handleDelete(id: string) {
    ...
    await fetch(`/api/admin/lands/${id}`, { method: "DELETE" });
    setSelectedIds(new Set());
    mutate();
  }
  ...
  async function handleBulkDelete() {
    ...
    setDeleting(true);
    try {
      await fetch("/api/admin/lands/bulk-delete", {
        method: "POST",
  ...
  async function handleDuplicate(l: Land) {
    const nextNum =
      lands.reduce(
        (max, item) => Math.max(max, Number(item.landId.replace("PL-", ""))),
        1000,
      ) + 1;
  ...
  return (
    <div>
      <div className="flex items-center justify-between gap-4">
        <div>
          <h1 className="text-2xl font-extrabold sm:text-3xl">Land</h1>
        ...
        <>
          ...
          {filtered.length === 0 ? (
            <p className="mt-12 text-center text-sm" style={{ color: "var(--color-ink-faint)" }}>
              {lands.length === 0 ? "No land listings yet" : "No land listings match the selected filters"}
            </p>
          ) : (
          <div className="mt-4">
